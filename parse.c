...
/*   Updated: 2024/07/08 13:23:04 by jeandrad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parse(int argc, char **argv, t_args pipex)
{
	...
	i = 0;
	if (argc != 5)
		exit_error(ARG_ERROR, 1);
	pipex.infile = argv[1];
	pipex.outfile = argv[4];
	pipex.cmd = malloc(sizeof(char *) * 3);
	if (!pipex.cmd)
		exit_error(CMD_ERROR, 1);
	pipex.cmd[0] = argv[2];
	pipex.cmd[1] = argv[3];
	pipex.cmd[2] = NULL;
	if (access(pipex.infile, R_OK) == -1)
		exit_error(INFILE_ERROR, 1);
	if (access(pipex.outfile, W_OK) == -1)
		exit_error(OUTFILE_ERROR, 1);
	if (pipe(pipex.pipe_fd) == -1)
		exit_error(PIPE_ERROR, 1);
